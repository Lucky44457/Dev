...
class SmartTelegramBot:
    ...
    async def upload_with_pyrogram(self, file_path: str, user_id: int, target_chat_id: int, caption: str, topic_id: Optional[int] = None, edit_msg=None):
        ...
        try:
            ...
            await app.send_message(None, f"**Pyrogram Upload Failed:** {str(e)}")
            raise
        finally:
            if edit_msg:
                try:
    ...
    async def upload_with_telethon(self, file_path: str, user_id: int, target_chat_id: int, caption: str, topic_id: Optional[int] = None, edit_msg=None):
        ...
        try:
            ...
            await app.send_message(None, f"**SpyLib Upload Failed:** {str(e)}")
            raise
    ...
    async def handle_large_file_upload(self, file_path: str, sender: int, edit_msg, caption: str):
        ...
        try:
            ...
            if free_check == 1:
                ...
                await app.copy_message(target_chat_id, result.id, protect_content=True, reply_markup=reply_markup)
            else:
                # Premium user - send normally
                await app.copy_message(target_chat_id, result.id)

        except Exception as e:
            print(f"Large file upload error: {e}")
            await app.send_message(None, f"**4GB Upload Error:** {str(e)}")
        finally:
            await edit_msg.delete()
    ...
    async def handle_message_download(self, userbot, sender: int, edit_id: int, msg_link: str, offset: int, message):
        ...
        try:
            ...
            await app.send_message(None, f"**Error:** {str(e)}")
        finally:
            # Cleanup
            if file_path:
    ...
    async def _copy_public_message(self, app_client, userbot, sender: int, chat_id: str, message_id: int, edit_id: int):
        """Public link handler (exact main.py style)."""
        target_chat_str = self.user_chat_ids.get(sender, str(sender))
        target_chat_id, topic_id = self.parse_target_chat(target_chat_str)

        try:
            # ✅ Always treat chat_id as username (like main.py)
            msg = await app_client.get_messages(str(chat_id), message_id)
            replace_caption = msg.caption

            # Apply caption replace if set
            rep = self.user_caption_prefs.get(str(sender))
            if rep and replace_caption:
                replace_caption = replace_caption.replace(rep, "")

            # ✅ Directly copy message using bot (bypasses forward restriction)
            result = await app_client.copy_message(
                target_chat_id,
                msg.chat.username or msg.chat.id,  # prefer username
                msg.id,
            ...
            # 🔄 Only if bot fails, fallback to userbot
            if not userbot:
                await app_client.send_message(sender, "❌ String session not set!")
                return

            edit_msg = await app_client.edit_message_text(sender, edit_id, "🔄 Trying alternative method...")
            msg = await userbot.get_messages(str(chat_id), message_id)
            if not msg:
                await edit_msg.edit("❌ Message not accessible")
                return
            ...
            if msg.document:
                result = await app_client.send_document(target_chat_id, file_path, caption=caption)
            elif msg.video:
                result = await app_client.send_video(target_chat_id, file_path, caption=caption)
            elif msg.photo:
                result = await app_client.send_photo(target_chat_id, file_path, caption=caption)
            else:
                result = await app_client.send_message(target_chat_id, caption or "")
